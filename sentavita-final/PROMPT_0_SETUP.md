# PROMPT 0 — Project Setup & Database
## Paste this into Cursor first. Complete before moving to PROMPT_1.

---

## Terminal commands — run these first
```bash
npx create-next-app@latest sentavita --typescript --tailwind --app --src-dir false --import-alias "@/*"
cd sentavita
...
## Create tailwind.config.ts — replace theme.extend entirely with:
```typescript
theme: {
  extend: {
    fontFamily: {
...
## Create the full folder structure (empty files)
```
app/
  globals.css
  layout.tsx
...
create index page_views_date_idx on page_views(viewed_at);
...
## Run this seed SQL in Supabase SQL Editor
```sql
